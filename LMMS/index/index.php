<?php
   ...
   @session_start() ;
   $_SESSION['scriptcase']['index']['glo_nm_perfil']          = "conn_mysql";
   $_SESSION['scriptcase']['index']['glo_nm_path_prod']       = "";
   $_SESSION['scriptcase']['index']['glo_nm_path_conf']       = "";
   $_SESSION['scriptcase']['index']['glo_nm_path_imagens']    = "";
   $_SESSION['scriptcase']['index']['glo_nm_path_imag_temp']  = "";
   $_SESSION['scriptcase']['index']['glo_nm_path_cache']      = "";
   $_SESSION['scriptcase']['index']['glo_nm_path_doc']        = "";
...
//
   function init($Tp_init = "")
   {
      ...
      $this->nm_dt_criacao   = "20210918"; 
      $this->nm_hr_criacao   = "123441"; 
      $this->nm_autor_alt    = "admin"; 
      $this->nm_dt_ult_alt   = "20211015"; 
      $this->nm_hr_ult_alt   = "182427"; 
...
// 
      ...
      if (!isset($_SESSION['scriptcase']['index']['save_session']['save_grid_state_session']))
      { 
          $_SESSION['scriptcase']['index']['save_session']['save_grid_state_session'] = false;
          $_SESSION['scriptcase']['index']['save_session']['data'] = '';
      } 
      ...
      $_SESSION['sc_session'][$this->sc_page]['index']['path_grid_sv'] = $this->root . substr($this->path_prod, 0, $pos_path) . "/conf/grid_sv/";
...
<?php
   ...
   function conectDB()
   {
      ...
      { 
          $this->Db = db_conect_devel($_SESSION['scriptcase']['index']['glo_nm_conexao'], $this->root . $this->path_prod, 'LMMS', 1, $this->force_db_utf8); 
      } 
      else 
      { 
          ob_start();
		...
		else     
{
	...
	$check_sql = "SELECT a.picture, b.picture "
	   . " FROM
	( SELECT id AS roww, picture FROM slider WHERE active = '1' AND ordernum % 2 <> 0 ) a
	LEFT JOIN ( SELECT id - 1 AS roww, picture FROM slider WHERE active = '1' AND ordernum % 2 = 0 ) b ON b.roww = a.roww";
...
$check_sqls = "SELECT picture, name, description "
	   . " FROM slider where active = '1'";
	 
      $nm_select = $check_sqls; 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_select; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      $this->rss = array();
      if ($SCrx = $this->Db->Execute($nm_select)) 
      { 
          ...
          { 
                 for ($SCx = 0; $SCx < $nm_count; $SCx++)
                 { 
                        $this->rss[$SCy] [$SCx] = $SCrx->fields[$SCx];
                 }
                 $SCy++; 
                 $SCrx->MoveNext();
          } 
...
$check_sqld = "SELECT name, picture, id "
	   . " FROM group_product where active = '1' ORDER BY ordernum ASC";
	 
      $nm_select = $check_sqld; 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_select; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      $this->rsd = array();
      if ($SCrx = $this->Db->Execute($nm_select)) 
      { 
          ...
          { 
                 for ($SCx = 0; $SCx < $nm_count; $SCx++)
                 { 
                        $this->rsd[$SCy] [$SCx] = $SCrx->fields[$SCx];
                 }
                 $SCy++; 
                 $SCrx->MoveNext();
          } 
      ...
      { 
          $this->rsd = false;
          $this->rsd_erro = $this->Db->ErrorMsg();
      } 
;
...
<link href="<?php echo sc_url_library('prj','web','assets/dist/css/headers.css') ?>" rel="stylesheet">
<link href="<?php echo sc_url_library('prj','web','assets/dist/css/product.css') ?>" rel="stylesheet">
	...
	body{
		background-image: url("<?php echo sc_url_library('prj','web','assets/dist/img/lmms.jpg') ?>");
...
<main>
		...
		<div id="carouselExampleCaptions" class="carousel slide" data-bs-ride="carousel">
			<div class="carousel-inner">
				...
				<div class="carousel-item <?php 
                        ...
                        ?>" data-interval="2000">
				<img src="../_lib/file/img/<?php echo $sliderss[0] ?>" class="d-block w-100">
				</div>
	...
	<div class="book-section">
    	<div class="containers">
			 <div class="right">
                <figure class="back" id="back-cover"><center><h1>Menu Book</h1>
                    <p><?php echo $slogan; ?></p></center></figure>
                <!--figure class="front" style="background-image: url(http://artisticdesigning.com/Drawings/Photoshopped/angry_man.jpg);"></figure-->
            </div>
<?php

	if(!empty($this->rs )){
	foreach($this->rs  as $sliders){
?>
	<div class="right">
                <figure class="back" style="background-image: url(../_lib/file/img/<?php echo $sliders[0] ?>);"></figure>
                <figure class="front" style="background-image: url(../_lib/file/img/<?php echo $sliders[1] ?>);"></figure>
            </div>								
	...
	<?php }} ?>
            ...
            <div class="right">
                ...
                <figure class="front" id="cover">
                    <center><h1>Menu Book</h1>
                    <p><?php echo $slogan; ?></p></center>
                </figure>
            </div>
        </div>
...
<div class="container marketing mb-2">

    <div class="row">
		<?php
				if(!empty($this->rsd )){
				foreach($this->rsd  as $group){
					
				$remove[] = " ";
				$merge = str_replace( $remove, "%20", $group[0]);
			?>
      ...
      <div class="col-6 col-md-4 text-center mt-4">
		...
		<div class="card" style="width: 140;">
		  <img width="140" src="../_lib/file/img/<?php echo $group[1] ?>" class="card-img-top" alt="<?php echo $group[0] ?>">
		  <div class="card-body">
			<h5 class="card-title text-center"><?php echo $group[0] ?></h5>
			  
			  <a href="https://api.whatsapp.com/send?phone=<?php echo $phone; ?>&text=Hi%2C%20I'd%20like%20to%20book%20appointment%20for%20Laboratory%20Order%20%F0%9F%91%87%0A%0A%E2%9C%85%20*SUMMARY%20VISIT*%0A%0A---------%0A%F0%9F%8F%AB%20Location%20(Select%20One)%20%3A%0A%0A---------%0A%F0%9F%A7%BE%20Examination%20%3A%0A<?php echo $merge; ?>%0A---------%0A%F0%9F%97%92%20Comment%0A%0A%0A%E2%9C%85%20Customer%20Info%0ANAME%20%7C%7C%20ADDRESS%20%7C%7C%20PHONE%20NUMBER%0AAvailable%20Time%20(fill%20below)%20%3A%0A00%3A00%20AM%2FPM%0A%0A%F0%9F%92%B3%20Payment%20Options%0AI%20would%20using%20(select%20one)%0A-%20Cash%0A-%20Online%20Transfer%0A-%20Insurance%20%3A%20Insurance%20Name%0A-%20Card%20%3A%20Institution%20Name%0A%0APlease%20take%20feedback%20soon%2C%20Thanks" class="btn btn-primary">
          Order
			</a>
  ...
  <!--div class="position-relative overflow-hidden p-3 p-md-5 m-md-3 text-center bg-light">
    <div class="col-md-5 p-lg-5 mx-auto my-5">
      ...
      <p class="lead fw-normal">Get interesting promos and benefits to join out as loyality membership.</p>
...
<div class="container">
  <div class="row">
    <div class="col-12 col-md">
		<img src="../_lib/file/img/<?php echo $logo;?>" alt="" width="64"/>
...
//======= =========================
   ...
   if ($Sem_Session && (!isset($nmgp_start) || $nmgp_start != "SC")) {
       ...
       elseif (is_file($root . $_SESSION['scriptcase']['index']['glo_nm_path_imag_temp'] . "/sc_apl_default_LMMS.txt")) {
           $apl_def = explode(",", file_get_contents($root . $_SESSION['scriptcase']['index']['glo_nm_path_imag_temp'] . "/sc_apl_default_LMMS.txt"));
       }
       if (isset($apl_def)) {
           if ($apl_def[0] != "index") {
           ...
           if (isset($_COOKIE['sc_actual_lang_LMMS'])) {
               $_SESSION['scriptcase']['index']['session_timeout']['lang'] = $_COOKIE['sc_actual_lang_LMMS'];
           }
       }
   }
