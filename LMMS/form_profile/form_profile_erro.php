...
$old_error_handler = set_error_handler("form_profile_scriptcase_error_handler");

function form_profile_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_profile']['contr_erro']) && $_SESSION['scriptcase']['form_profile']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_profile->contr_form_profile->Apl_com_erro = true;
            if ($inicial_form_profile->contr_form_profile->NM_ajax_flag)
            {
                $inicial_form_profile->contr_form_profile->NM_ajax_info['errList']['geral_form_profile'][] = $msg;
                return;
            }
            form_profile_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_profile_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_profile'])  && '' != $_SESSION['scriptcase']['error_icon']['form_profile'])  ? $_SESSION['scriptcase']['error_icon']['form_profile']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_profile']) && '' != $_SESSION['scriptcase']['error_close']['form_profile']) ? $_SESSION['scriptcase']['error_close']['form_profile'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_profile']) && '' != $_SESSION['scriptcase']['error_span_title']['form_profile']) ? $_SESSION['scriptcase']['error_span_title']['form_profile'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_profile']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_profile']) ? $_SESSION['scriptcase']['error_icon_title']['form_profile'] : '';
?>
