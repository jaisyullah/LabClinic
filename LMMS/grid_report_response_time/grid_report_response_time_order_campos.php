...
<?php
   ...
   $Ord_Cmp = new grid_report_response_time_Ord_cmp(); 
   $Ord_Cmp->Ord_cmp_init();
   
class grid_report_response_time_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $use_alias, $tab_ger_campos, $tab_def_campos, $tab_def_seq, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_report_response_time']['ordem_select_orig']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_report_response_time']['ordem_select_orig'] = array();
   }
   ...
   $tab_labels["trancode"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_report_response_time']['labels']["trancode"])) ? $_SESSION['sc_session'][$sc_init]['grid_report_response_time']['labels']["trancode"] : "Transaction Code";
   $tab_ger_campos['processed'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['processed'] = "processed";
   }
   else
   {
       $tab_def_campos['processed'] = "a.time";
   }
   $tab_labels["processed"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_report_response_time']['labels']["processed"])) ? $_SESSION['sc_session'][$sc_init]['grid_report_response_time']['labels']["processed"] : "Processed";
   $tab_ger_campos['confirmed'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['confirmed'] = "confirmed";
   }
   else
   {
       $tab_def_campos['confirmed'] = "b.time";
   }
   $tab_labels["confirmed"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_report_response_time']['labels']["confirmed"])) ? $_SESSION['sc_session'][$sc_init]['grid_report_response_time']['labels']["confirmed"] : "Confirmed";
   $tab_ger_campos['finished'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['finished'] = "finished";
   }
   else
   {
       $tab_def_campos['finished'] = "c.time";
   }
   $tab_labels["finished"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_report_response_time']['labels']["finished"])) ? $_SESSION['sc_session'][$sc_init]['grid_report_response_time']['labels']["finished"] : "Finished";
   $tab_ger_campos['confirm'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['confirm'] = "confirm";
   }
   else
   {
       $tab_def_campos['confirm'] = "ROUND( time_to_sec(( TIMEDIFF( b.time,a.time ))) / 60 )";
   }
   $tab_labels["confirm"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_report_response_time']['labels']["confirm"])) ? $_SESSION['sc_session'][$sc_init]['grid_report_response_time']['labels']["confirm"] : "Confirm (in Min)";
   $tab_ger_campos['finish'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['finish'] = "finish";
   }
   else
   {
       $tab_def_campos['finish'] = "ROUND( time_to_sec(( TIMEDIFF( c.time,a.time ))) / 60 )";
   }
   $tab_labels["finish"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_report_response_time']['labels']["finish"])) ? $_SESSION['sc_session'][$sc_init]['grid_report_response_time']['labels']["finish"] : "Finish (in Min)";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_report_response_time']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_report_response_time']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_report_response_time']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_report_response_time']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_report_response_time']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_report_response_time']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_report_response_time']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_report_response_time']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       if (isset($tab_def_campos[$campo]))
       {
           ...
           if (isset($_SESSION['sc_session'][$sc_init]['grid_report_response_time']['ordem_quebra']))
           {
               foreach($_SESSION['sc_session'][$sc_init]['grid_report_response_time']['ordem_quebra'] as $campo_quebra => $resto) 
               {
                   foreach($resto as $sqldef => $ordem_ant) 
                   {
                       if ($sqldef == $tab_def_campos[$campo]) 
                       { 
                           $_SESSION['sc_session'][$sc_init]['grid_report_response_time']['ordem_quebra'][$campo][$sqldef] = $ordem;
           ...
           if (!$Ordem_tem_quebra)
           {
               $_SESSION['sc_session'][$sc_init]['grid_report_response_time']['ordem_select'][$tab_def_campos[$campo]] = $ordem;
           }   
       }
   }
?>
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   return new Promise(function(resolve, reject) {$.ajax({
    type: "POST",
    url: "grid_report_response_time_order_campos.php",
    data: {
...
<?php
   ...
   $arr_order = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_report_response_time']['ordem_select_orig'] : $_SESSION['sc_session'][$sc_init]['grid_report_response_time']['ordem_select'];
...
<?php
  $disp_rest = "none";
  if (isset($_SESSION['sc_session'][$sc_init]['grid_report_response_time']['ordem_select']) && $_SESSION['sc_session'][$sc_init]['grid_report_response_time']['ordem_select'] != $_SESSION['sc_session'][$sc_init]['grid_report_response_time']['ordem_select_orig']) {
      $disp_rest = "";
  }
   if (!$embbed)
   {
